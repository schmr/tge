...
{
	std::string fname = ArgPack::ap().get_fname();
	std::string str = "kmake.perl " + fname;
...
{
	std::ifstream annote_stream;
	rstd::RVector<std::string> vec;
	rstd::RVector<long> time_s;
	rstd::RVector<long> time_us;
	std::string s;
	...
	do {
		...
		Rassert(!vec.empty());
		if (!(vec[0] == "KSV")) {
			type_map_[vec[0]] = rstd::Conv(vec[1]);
		}	
	} while (!(vec[0] == "KSV"));

	// Read data profile times.  Each line is read in as a
	// tokenized std::string (with 5 components).
	do {
		getline(annote_stream, s);
		vec = rstd::tokenize (s);
		if (!vec.empty()) {
	...
	rstd::RVector<CallStackItem> fn_stack;
	rstd::RVector<CallStackItem> loop_select_stack;
	MAP (x, annote_data_.size()) {
		...
		std::string fname = function_[fn].name();
		...
		while (fn_stack.size()) {
			...
			t2.set_time(rstd::Conv(annote_data_[x][3]), rstd::Conv(annote_data_[x][4]));
			fg_[f].set_time_unit(t2-t1);
//			std::cout << "FTIME: " << function_[f].name() << " " << t2-t1 << std::endl;
			fn_stack.pop_back();
		}
...
//			std::cout << "LC: " << loop_select_stack[loop_select_index].tree_node()
//					<< " " << t2-t1 << std::endl;
		...
		if (annote_data_[x][1] == "statement_end") {
			t1.set_time(rstd::Conv(annote_data_[x-1][3]), rstd::Conv(annote_data_[x-1][4]));
			t2.set_time(rstd::Conv(annote_data_[x][3]), rstd::Conv(annote_data_[x][4]));
			set_se_task_time (fn, rstd::Conv(annote_data_[x][2]), t2-t1);
//			std::cout << "SE: " << annote_data_[x][2] << " " << t2-t1 << std::endl;

		} else if (annote_data_[x][1] == "fn_start") {
			long tree_node = rstd::Conv(annote_data_[x][2]);
			long f = func_map(annote_data_[x][0]);
			Rassert(f != -1);
			t1.set_time(rstd::Conv(annote_data_[x][3]), rstd::Conv(annote_data_[x][4]));
...
void DB::set_all_fc_task_times ()
{
	...
	MAP (x, function_.size()) {
		rstd::RVector<long> func_node = fg_[x].func_call_nodes();
		MAP (y, func_node.size()) {
			TimeUnit tu;
			tu.set_time(0,0);
			rstd::RVector<long> call_list = fg_[x][func_node[y]].func();
...
void DB::prof_get_arc_lengths()
{
	...
	MAP (x, variable_.size()) {
		std::string type_name = variable_[x].type();
		long type_size;
//std::cout << variable_[x].name() << " " << variable_[x].type();
...
//		std::cout << " " << variable_[x].total_size() << std::endl;
	}
...
//std::cout << function_[x].name() << ": " << bob  << std::endl;
//}
...
//			std::cout << function_[x].name() << " node:" << y << " ";
			...
			MAP (z, fg_[x].vertex(y)->size_out()) {
				...
				std::set<long> i_set = fg_[x](fg_[x].vertex(y)->out(z)).vars();
...
//std::cout << variable_[var_index].name() << " ";
				}
//std::cout << std::endl;
				i_set.clear();
...
long DB::determine_node_function_out_size (long f, long n)
{
	...
	//--------------------------------------------------
	long sz = 0;
	rstd::RVector<long> func = fg_[f][n].func();

//	std::cout << function_[f].name() << " node: " << n << ":  ";

	MAP (x, func.size()) {
//		std::cout << function_[func[x]].name() << " ";
		std::string type = function_[func[x]].type();
...
{
	std::ofstream out;
	std::string filename = ArgPack::ap().get_fname() + ".tg";
	out.open(filename.c_str());

	out << "#  " << filename << " Task Graphs:" << std::endl;
	out << "#  (Automatically generated file)" << std::endl << std::endl;

	MAP (x, fg_.size()) {
		out << "#  " << fg_[x].name() << std::endl;
		if (fg_[x].name() == "main") {
			out << "@Task Graph " << x << "  {" <<std::endl;
		} else {
			out << "@SubTask Graph " << x << "  {" <<std::endl;
		}
//		out << "SE: " << fg_[x].statement_nodes() << std::endl;
//		out << "LC: " << fg_[x].loop_or_cond_nodes() << std::endl;
//		out << "FC: " << fg_[x].func_call_nodes() << std::endl;
...
{
	std::ofstream out;
	std::string filename = ArgPack::ap().get_fname() + ".std_tg";
	out.open(filename.c_str());

	out << "#  Task graph for --  " << filename << ":" << std::endl;
	out << "#  (Automatically generated file)" << std::endl << std::endl;
	out << "@HYPERPERIOD 1" << std::endl << std::endl;

	std::string main_string = "main";
	rstd::RVector<long> fn_list = call_graph_.top_sort (func_map_[main_string]);
	rstd::RVector<TimeUnit> task_type;
	rstd::RVector<long> arc_type;
	...
	MAP (x, fn_list.size()) {
		...
		MAP (y, fg_[curr_func].size_edge()) {
			...
			if (arc_type[arc_type.size() - 1] < 0) {
				std::cerr << "Edge with negative value.  "
					<< "Probably caused by pointer."  << std::endl;
				Rabort();
			}
		}
	}
	out << "@COMMUN_QUANT 0 {" << std::endl;
	MAP (x, arc_type.size()) {
		out << x << "\t" << arc_type[x] << std::endl;
	}
	out << "}\n\n";
	...
	MAP (x, fn_list.size()) {
		...
		out << "#  " << fg_[curr_func].name() << std::endl;
		if (is_main) {
			out << "@TASK_GRAPH " << x << "  {" << std::endl;
			out << "  PERIOD 1" << std::endl;
		} else {
			out << "# @SUBTASK_GRAPH " << x << "  {" << std::endl;
			out << "#  APERIODIC " << std::endl;
		}
		...
		MAP (y, fg_[curr_func].size_vertex()) {
			...
			out << "   TASK t" << x << "_" << y;
			out << "   TYPE " << task_type.size() << std::endl;
	...
	out << "# price buffered max_freq width    height   density "
		<< " preempt_power commun_en_bit io_en_bit idle_power" << std::endl;
